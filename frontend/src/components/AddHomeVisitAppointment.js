...
const AddHomeVisitAppointment = ({
  ...
  {/** create channeling appointment when click*/}
  const handleSubmit = async(e) =>{
    e.preventDefault()

    const response = await fetch(SummaryApi.AddHomeVisitAppointment.url,{
      method : SummaryApi.AddHomeVisitAppointment.method,
      credentials : 'include',
      headers : {
  ...
  return (
    <div className='fixed w-full h-full bg-slate-200 top-0 left-0 right-0 bottom-0 flex justify-center items-center bg-opacity-70 rounded-3xl'>
       <div className='bg-white p-4 rounded-xl w-full max-w-2xl h-full max-h-[90%] pb-10'>

        <div className='flex justify-between items-center pb-1'>
           <h2 className='font-bold text-lg items-center'>
              Add Home Visit Appointment
           </h2>
        ...
        <form className='grid p-4 gap-4 overflow-y-scroll h-full' onSubmit={handleSubmit}>
           ...
           <select id='doctorName' name='doctorName' value={data.doctorName} onChange={handleOnChange} className='p-2 bg-slate-100 border rounded' required>
            ...
            <textarea 
                ...
                name='otherNotes'
                onChange={handleOnChange}
                className='h-28 bg-slate-100 border rounded resize-none' rows={3}
      
                />

        
        <button className='px-3 py-2 bg-blue-600 text-white hover:bg-blue-800 mt-10 rounded-full'>Create Appointment</button>
...
export default AddHomeVisitAppointment
