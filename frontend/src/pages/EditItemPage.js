...
const EditItemPage = () => {
  const { id } = useParams();
  const [item, setItem] = useState({ itemId: '', itemName: '', itemCount: 1 });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setTimeout(() => navigate('/all-items'), 1000); // Redirect after 2 seconds
      } else {
    ...
    <div className="max-w-4xl mx-auto p-6 bg-white shadow-lg rounded-lg mt-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="flex flex-col">
          <label className="text-gray-600 font-medium mb-2">Item ID:</label>
          <input
            type="text"
            value={item.itemId}
